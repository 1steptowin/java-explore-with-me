...
public class CategoryServiceImpl implements CategoryService{
    ...
    public CategoryDto updateCategory(Long catId, CategoryDto inputCategory) {
        Category updated = categoryRepo.findById(catId).orElseThrow(() -> {
            throw new CategoryNotFoundException(String.format("Category %d does not exist", catId));
        });
        updated.setName(inputCategory.getName());
        return CategoryMapper.mapModelToDto(categoryRepo.save(updated));
    }
